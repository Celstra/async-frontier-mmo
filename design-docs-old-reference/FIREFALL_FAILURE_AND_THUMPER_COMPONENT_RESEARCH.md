...
- `FIREFALL_THUMPER_GROUP_VS_PERSONAL_RESEARCH.md`
- `SWG_RESOURCE_CRAFTING_FEEDBACK_RESEARCH.md`
- `DESIGN_BIBLE.md`

The earlier examples about capped node yield, recovery efficiency, owner reserve, and contribution-weighted splits are documented in `FIREFALL_THUMPER_GROUP_VS_PERSONAL_RESEARCH.md`.
...
- Make the first durable spine: survey → extract → refine → craft → use/decay → repeat.
...
Start with only five SWG-like stats:
...
This is enough to test whether crafting different thumper parts creates meaningful choices.
