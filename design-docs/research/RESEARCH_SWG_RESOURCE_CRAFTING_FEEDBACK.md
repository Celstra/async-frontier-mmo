...
Post-MVP note: the locked MVP has one basic personal thumper only. Do not copy Firefall's capacity numbers directly when later tiers are considered. Use conservative tiers.
...
Final locked MVP correction: start smaller than the earlier candidate list.
...
- No public marketplace, no group thumpers, no mandatory refining, and no Chemical Purity in the MVP.
- Ledger every economy-relevant mutation.
