...
## 8. Recommended MVP / post-MVP implementation

Do not launch with group-thumper complexity. The locked MVP uses one basic personal thumper only; group thumpers are a post-MVP layer.

### MVP personal thumper

- One basic personal thumper only.
- Owner receives output.
- Drill / Pump / Hull are the only component slots.
- Default runs have 2 event windows; high-risk/push runs may have up to 3.
- Complications are Signal Drift, Hull Damage, Threat Surge, and Pump Strain.
- Recall Early is always available as a universal safety/resolution choice.

### Post-MVP group thumper test
